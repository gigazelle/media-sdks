...
This is the same scenario as [VOD playback with pre-roll ads](vod-preroll-ads.md), except the application has a provision to let the user skip the ad, on the click of a skip button perhaps.  

| Trigger | Heartbeat method | Network calls | Notes |
| --- | --- | --- | --- |
| User clicks **Play** | `trackSessionStart()` | Analytics Content Start, Heartbeat Content Start | The measurement library is unaware that there is a pre-roll ad. These network calls are still exactly the same as [VOD playback with no ads](vod-no-intrs-details.md) scenario. |
...
| The content plays.  | | Content Heartbeats | These network calls are exactly the same as the [VOD playback with no ads](vod-no-intrs-details.md) scenario. |
| The content completes playing.  | `trackComplete()` | Heartbeat Content Complete | This network call is exactly the same as the [VOD playback with no ads](vod-no-intrs-details.md) scenario. |
| The session is over.  | `trackSessionEnd()` | | `SessionEnd` |

## Parameters

The parameters are identical to the parameters in the [VOD playback with pre-roll ads](vod-preroll-ads.md) scenario, except there is no ad complete call. `AdBreakComplete` is still required after the skip to close the ad break.

## Sample Code

![](../../../help/use-cases/scenarios/assets/ad-skip.png)
