...
In this scenario, there is one live asset with no ads played for 40 secs after joining the live stream, with sequential episode tracking.

| Trigger | Heartbeat method | Network calls | Notes |
| --- | --- | --- | --- |
| User clicks **Play** | `trackSessionStart` | Analytics Content Start, Heartbeat Content Start | |
...
During media playback, there is a timer that will send one or more heartbeats every 10 seconds for main content, and every one second for ads. These heartbeats will contain information about playback, ads, buffering, and a number of other things.
...
When playback for any given episode has completed (playhead crosses episode boundary), a Heartbeat Content Complete call is sent. This looks like other Heartbeat Calls, but will contain a couple specific things:
...
![](../../../help/use-cases/scenarios/assets/ios-live-noads-multiplesessions.png)
...
// There is no need to reinstantiate a mediaHeartbeat instance for tracking session 2. 
// Set up mediaObject 
MediaObject mediaInfo = MediaHeartbeat.createMediaObject( 
  Configuration.MEDIA_NAME,  
  Configuration.MEDIA_ID,  
...
// reinstantiate mediaHeartbeat instance for tracking session 2. 
...
//    i.e., when playback completes and finishes playing the 1st episode/session. 
...
// to reinstantiate a mediaHeartbeat instance for tracking session 2. 
